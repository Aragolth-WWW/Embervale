...
- Follow-up (2025-11-14): Removed the temporary owner-debug logs and `G` key fallback once the prefab consistently spawns with `SimpleAnimatorDriver` via `[RequireComponent]`. All attack pulses now route exclusively through the Input System action map.
